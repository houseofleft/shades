...
def gradient(
    ...
    def shade(xy: Tuple[int, int], width: int, height: int) -> np.ndarray:
        ...
        """
        noise_ranges = np.array([i.noise_range(xy, width, height) for i in color_fields])
...
def custom_shade(custom_function: Callable[Tuple[int, int], Tuple[int, int, int]]) -> Callable:
    """
